  ...
  data_type = {
    'Image': {'page1': {text: "<H2>Instrucciones (1/3)</H2>A continuación te pediremos que des recomendaciones sobre situaciones hipotéticas. Se trata de personas que llegan a una consulta rutinaria, preguntando por un cribado, o a urgencias con sospecha de una condición grave. " +
              ...
              'page2': {text: "<H2>Instrucciones (2/3)</H2>Usando la prevalencia, sensibilidad y especificidad es posible calcular el Valor Predictivo Positivo o Negativo. Esto es, la probabilidad de tener la enfermedad cuando el test da positivo (Valor Predictivo Positivo) o la probabilidad de NO tener la enfermedad cuando el test da negativo (Valor Predictivo Negativo).<BR><BR>" +
                              ...
                              '<img src="media/img/FONDECYT/instructions.png" style="max-width: 40%; max-height: 40%;"> <img src="media/img/FONDECYT/instructions2.png" style="max-width: 40%; max-height: 40%;">' +
                              "<BR><BR>"},
...
// ------------------------------------------------------------------------------------------------
  ...
  data_disease = {
    'Cancer': {description_context1: "Imagina que acude a tu consulta preguntando por el <B>screening para cáncer de mama</B> ",
              description_context2: " de 40 años de raza caucásica y peso normal. Su estado de salud es aparentemente normal.<BR><BR>Su historia clínica no presenta antecedentes familiares de cáncer, no consume alcohol ni drogas habitualmente.<BR>",
              brief_context: " de 40 años sin antecedentes familiares de cáncer acude a consulta preguntando por el <B>screening para cáncer de mama</B>.<BR>",
    ...
    'Stroke': {description_context1: "Imagina que acude a urgencias con <B>sospecha de infarto cerebral</B> ",
              description_context2: " de 40 años de raza caucásica y peso normal. Presenta una disartria moderada (tiene dificultad para articular palabras pero con esfuerzo se puede comprender lo que dice).<BR><BR>Su historia clínica no presenta antecedentes familiares de infarto cerebral, no consume alcohol ni drogas habitualmente.<BR>",
              brief_context: " de 40 años acude a urgencias con disartria moderada: <B>sospecha de infarto cerebral</B>.<BR>",
...
// --------------------------------------------------------------------------------------
  ...
  var within_timeline_01 = {
    timeline: [
      ...
      // SCREEN 4: Confidence in Recommendation 1 ------------------------------
      {
        ...
        stimulus: function () {
          answers = [Object.values( JSON.parse( (jsPsych.data.get().filter({trialid: 'FONDECYT_02_' + num})).select('response').values[(jsPsych.data.get().filter({trialid: 'FONDECYT_02_' + num})).select('response').values.length - 1] ) ),
                    Object.values( JSON.parse( (jsPsych.data.get().filter({trialid: 'FONDECYT_03_' + num})).select('response').values[(jsPsych.data.get().filter({trialid: 'FONDECYT_03_' + num})).select('response').values.length - 1] ) )];
  ...
  FONDECYT.unshift(if_instructions_000);
