...
var question000 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">Por favor, ingrese su ID de usuario otorgada por Prolific</div>', type: 'text', range: [-Infinity, Infinity], required: true}],
...
var question00 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">Indique el país de su nacionalidad</div>', type: 'text', range: [-Infinity, Infinity], required: true}],
...
var question01 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">Para continuar ingrese su rut Completo (sin puntos ni guión y con dígito verificador). <br> Si su rut termina con K, reemplácelo por un 0</div>', type: 'number', range: [10000000, 999999999], required: true}],
...
var question03 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">Nombre completo</div>', type: 'text', range: [-Infinity, Infinity], required: true}],
...
var question04 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">Edad</div>', type: 'number', range: [0, 120], required: true}],
...
var question06 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">Nivel educacional en años de estudio <BR><BR> (Considere hasta el año realizado. Por ejemplo, si terminó 4to medio y no realizó nada más, poner 12 años. Si realizo dos años de formación técnica después de terminar 4to medio, poner 14 años. Si llegó solo hasta 3er medio, poner 10 ya que terminó 2do medio. etc.)</div>', type: 'number', range: [0, 50], required: true}],
...
var question08 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">Introduce tu número de celular con el formato 912345678 (opcional)</div>', type: 'number', range: [900000000, 9999999999], required: false}],
...
var question10 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">Indicar el año en que usted fue diagnosticado y tipo de diagnóstico (por ejemplo: 2018 ansiedad, 2019 depresión, 2020 esquizofrenia)</div>', type: 'text', range: [-Infinity, Infinity], required: true}],
...
var if_question10 = {
  ...
  conditional_function: function(){
    let data = (JSON.parse((jsPsych.data.get().values().find(x => x.trialid === 'FORM5_09'))['response'])['Q0']).trim();
    if((data) ==  'Sí'){
      return true;
    } else {
      return false;
...
var question12 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">¿Qué tipo de tratamiento?</div>', type: 'text', range: [-Infinity, Infinity], required: true}],
...
var if_question12 = {
  ...
  conditional_function: function(){
    let data = (JSON.parse((jsPsych.data.get().values().find(x => x.trialid === 'FORM5_11'))['response'])['Q0']).trim();
    if((data) ==  'Sí'){
      return true;
    } else {
      return false;
...
var question14 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">¿Qué tipo de droga ha consumido?</div>', type: 'text', range: [-Infinity, Infinity], required: true}],
...
var if_question14 = {
  ...
  conditional_function: function(){
    let data = (JSON.parse((jsPsych.data.get().values().find(x => x.trialid === 'FORM5_13'))['response'])['Q0']).trim();
    if((data) ==  'Sí'){
      return true;
    } else {
      return false;
