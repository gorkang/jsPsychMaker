...
function XMLcall(
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (query == "createTable") {
        ...
        } else if (query == "updateTable") {
            //update "1" element from mysql table
            base_query.id = "";
            for (var [key, value] of Object.entries(elements.id)) {
                base_query.id = key + "=";
                if (
                    value.toString().indexOf("+ 1") != -1 ||
                    value.toString().indexOf("- 1") != -1 ||
                    isNormalInteger(value.toString())
                ) {
                    base_query.id += value;
                } else {
                    base_query.id += '"' + value + '"';
            ...
            if (!Object.entries(elements.sql)[0][1].startsWith("SELECT"))
                throw new Error("general_query ERROR");

            // Put elements in SQL query
            base_query.id =
                Object.entries(elements.id)[0][0] +
                "=" +
                Object.entries(elements.id)[0][1];
            base_query.sql = Object.entries(elements.sql)[0][1];

            //console.log(base_query);
        } else if (query == "findRow" || query == "findAll") {
            if ("keys" in elements) base_query.keys = elements.keys;
            if ("values" in elements) base_query.values = elements.values;
        }
...
function start_mysqldb(pid, max_participants) {
    ...
    if (debug_mode === true) {
        ...
        XMLcall("createTable", "protocol", {
            keys:
                "id_protocol INT NOT NULL PRIMARY KEY DEFAULT 0, counter INT NOT NULL DEFAULT 0, last_revision TIMESTAMP DEFAULT CURRENT_TIMESTAMP, max_participants INT NOT NULL DEFAULT " +
                max_participants.toString()
        });
        XMLcall("createTable", "experimental_condition", {
            keys:
                "id_condition INT AUTO_INCREMENT NOT NULL PRIMARY KEY, id_protocol INT NOT NULL DEFAULT 0, assigned_task INT NOT NULL DEFAULT 0, blocked BOOLEAN NOT NULL DEFAULT 0, completed_protocol INT NOT NULL DEFAULT 0, condition_key VARCHAR(255) NOT NULL, condition_name VARCHAR(255) NOT NULL, task_name VARCHAR(255) NOT NULL, type VARCHAR(255), UNIQUE KEY unique_combination (id_protocol, condition_name)"
        });

        // necesario para la lista de tareas completadas
        XMLcall("createTable", "task", {
            keys:
                "id_task INT AUTO_INCREMENT NOT NULL PRIMARY KEY, id_protocol INT NOT NULL DEFAULT 0, task_name VARCHAR(255) NOT NULL, UNIQUE KEY unique_combination (id_protocol, task_name)"
        });

        XMLcall("createTable", "user", {
            keys:
                "id_user INT AUTO_INCREMENT NOT NULL PRIMARY KEY, id_protocol INT NOT NULL DEFAULT 0, uid_external VARCHAR(255) NOT NULL,  start_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP, status VARCHAR(255) NOT NULL DEFAULT 'assigned', UNIQUE KEY unique_combination (id_protocol, uid_external)"
        });

        XMLcall("createTable", "user_condition", {
            keys:
                "id INT AUTO_INCREMENT NOT NULL PRIMARY KEY, id_protocol INT NOT NULL DEFAULT 0, id_condition INT NOT NULL REFERENCES experimental_condition(id_condition), id_user INT NOT NULL REFERENCES user(id_user), UNIQUE KEY unique_combination (id_protocol, id_condition, id_user)"
        });
        XMLcall("createTable", "user_task", {
            keys:
                "id INT AUTO_INCREMENT NOT NULL PRIMARY KEY, id_protocol INT NOT NULL DEFAULT 0, id_task INT NOT NULL REFERENCES task(id_task), id_user INT NOT NULL REFERENCES user(id_user), UNIQUE KEY unique_combination (id_protocol, id_task, id_user)"
        });
    }

    // CHECK if task table has contents [this select is tied to the pid]
    XMLcall("findRow", "task", {
        keys: ["task_name"],
        values: ["Consent"]
    }).then(function(tasks_in_table) {
    ...
    XMLcall("insertIntoTable", "protocol", {
        dict: { id_protocol: pid, max_participants: max_participants }
    });
    for (var j in conditions) {
        XMLcall("insertIntoTable", "experimental_condition", {
            dict: conditions[j]
        });
    }
}
    ...
    XMLcall("findAll", "user").then(
        function(users) {
            ...
            for (var i = 0; i < users.length; i++) {
                ...
                seconds_since_start =
                    (new Date(actual_time) - new Date(DBtime)) / 1000;
                ...
                if (
                    ...
                    XMLcall("updateTable", "user", {
                        id: { id_user: users[i].id_user },
                        data: { status: "discarded" }
                    });
                    XMLcall("updateTable", "protocol", {
                        id: { id_protocol: pid },
                        data: { counter: "counter - 1" }
                    });

                    // UPDATE assigned_task -1 for each between_selection condition
                    XMLcall("findAll", "user_condition", {
                        keys: ["id_user"],
                        values: [users[i].id_user]
                    }).then(function(user_conditions) {
                        for (var i = 0; i < user_conditions.length; i++) {
                            XMLcall("updateTable", "experimental_condition", {
                                id: {
                                    id_condition:
                                        user_conditions[i].id_condition
                                },
                                data: { assigned_task: "assigned_task - 1" }
                            });
                        }
                    });
                    ...
                    if (debug_mode === true)
                        console.warn(
                            'clean_mysql() || seconds_since_start > max_sec && users[i].status == "assigned" || UPDATE || status: discarded, counter - 1, for(assigned_task - 1) | users.id_user: ' +
                                users[i].id_user +
                                "(uid_external " +
                                users[i].uid_external +
                                ")"
...
function load_clean_mysql(iterations_for_review, max_participants) {
    ...
    XMLcall("findAll", "experimental_condition").then(
        function(condition_data) {
            // DB clean with clean_mysql()
            XMLcall("findRow", "protocol", {
                keys: ["id_protocol"],
                values: [pid]
            }).then(
                function(actual_stats) {
            ...
            for (var i = 0; i < condition_data.length; i++) {
                if (condition_data[i].assigned_task >= max_participants) {
                    ...
                    XMLcall("updateTable", "experimental_condition", {
                        id: { id_condition: condition_data[i].id_condition },
                        data: { blocked: true }
                    });
                    if (debug_mode === true)
                        console.warn(
                            "load_clean_mysql() | UPDATE | for(blocked: true) | condition_data[i].assigned_task >= max_participants"
                        );
                } else {
                    ...
                    if (debug_mode === true)
                        console.warn(
                            "load_clean_mysql() | UPDATE | for(blocked: false) | ELSE: condition_data[i].assigned_task >= max_participants"
                        );
                }
            }
    ...
    return new Promise(function(resolve, reject) {
        XMLcall("findAll", "experimental_condition").then(
            function(condition_data) {
                ...
                if (debug_mode === true)
                    console.warn(
                        "condition_selection() || Object.keys(between_selection_temp).length: " +
                            Object.keys(between_selection_temp).length
                    );
                if (debug_mode === true)
                    console.warn(
                        "between_selection_temp: '" +
                            Object.keys(between_selection_temp) +
                            "'"
                    );
                // FOR DEBUG:
                /*
        ...
        */
                ...
                if (Object.keys(between_selection_temp).length === 0) {
                    ...
                    unique_between_tasks = [
                        ...new Set(condition_data.map(item => item.task_name))
                    ];
                    ...
                    if (unique_between_tasks.length === 0) {
                        if (debug_mode === true)
                            console.error(
                                "condition_selection() | unique_between_tasks EMPTY | No between_tasks defined | continuing: we assume is a protocol without between conditions"
                            );
                        resolve(true);
                    } else {
                        ...
                        for (var i = 0; i < unique_between_tasks.length; i++) {
                            for (var j = 0; j < condition_data.length; j++) {
                                if (
                                    condition_data[j].task_name ==
                                    unique_between_tasks[i]
                                ) {
                                    condition_data[j].assigned_task = parseInt(
                                        condition_data[j].assigned_task
                                    );
                                }
                            }

                            // Temporal array for the condition i
                            ARRAY_between_temp[i] = condition_data.filter(
                            ...
                            min_assigned_temp = Math.min.apply(
                                Math,
                                ARRAY_between_temp[i].map(function(
                                    value,
                                    index
                                ) {
                                    return value["assigned_task"];
                                })
                            );
                            ...
                            available_conditions_ARRAY = ARRAY_between_temp[
                                i
                            ].filter(function(value, index) {
                                return (
                                    value["assigned_task"] <=
                                        min_assigned_temp &&
                                    value["assigned_task"] < max_participants
                                );
                            });
                            ...
                            if (available_conditions_ARRAY.length > 0) {
                                if (debug_mode === true)
                                    console.warn(
                                        "condition_selection() | Choosing between the " +
                                            available_conditions_ARRAY.length +
                                            " conditions available."
                                    );

                                grouped_avaliable_conditions = groupBy(
                                ...
                                for (let avaliable_condition in grouped_avaliable_conditions) {
                                    randomly_selected_index = jsPsych.randomization.sampleWithoutReplacement(
                                        Array(
                                            grouped_avaliable_conditions[
                                                avaliable_condition
                                            ].length
                                        )
                                            .fill()
                                    ...
                                    condition_data_temp.push(
                                        grouped_avaliable_conditions[
                                            avaliable_condition
                                        ][randomly_selected_index]
                                    );

                                    if (debug_mode === true) {
                                        // Create simple versions to print in console
                                        SIMPLE_ARRAY = ARRAY_between_temp[0].map(
                                            function(item) {
                                                return {
                                                    ...
                                                    completed_protocol: [
                                                        item.completed_protocol
                                                    ]
                                                };
                                            }
                                        );
                                        SIMPLE_ARRAY_CHR = JSON.stringify(
                                            flattenObject(SIMPLE_ARRAY)
                                        )
                                            .replace(/","1\.|","2\./, " \n ")
                                            .replace(
                                                /0\.|\.0|1\.|\.1|2\.|\.2/g,
                                        ...
                                        available_conditions_SIMPLE_ARRAY = grouped_avaliable_conditions[
                                            avaliable_condition
                                        ].map(function(item) {
                                            return {
                                                task_name: [item.task_name],
                                        ...
                                        available_conditions_SIMPLE_ARRAY_CHR = JSON.stringify(
                                            flattenObject(
                                                available_conditions_SIMPLE_ARRAY
                                            )
                                        )
                                            .replace(/","1\.|","2\./, " \n ")
                                            .replace(
                                ...
                                if (debug_mode === true)
                                    console.warn(
                                        "condition_selection() | No available conditions"
                                    );
                            }
                            ...
                            // No slots available
                            if (
                                condition_data_temp === undefined ||
                                condition_data_temp.length == 0
                            ) {
                                experiment_blocked = true;
                                condition_temp_array = [false]; // REVIEW: Needed here?
                                if (debug_mode === true)
                                    console.warn(
                                        "condition_selection() | Final check NEW | No slots available"
                                    ); // Ends up in jsPsych.end
                                ...
                                between_selection[unique_between_tasks[i]] = {};
                                console.log(condition_data_temp);
                                for (let element of condition_data_temp) {
                                    between_selection[unique_between_tasks[i]][
                                    ...
                                    if (debug_mode === true)
                                        console.warn(
                                            "condition_selection() | Final check NEW | Available slots"
                                        );
                                    resolve(true);
                                }
                            }
                    ...
                    XMLcall("findRow", "user", {
                        keys: ["uid_external"],
                        values: [uid_external]
                    }).then(function(actual_user) {
                        if (debug_mode === true)
                            console.warn(
                                "condition_selection() | ELSE: Object.keys(between_selection_temp).length === 0"
                            );
                        ...
                        Object.entries(between_selection_temp).forEach(
                            ([key, val]) => {
                                if (debug_mode === true)
                                    console.warn(
                                        "condition_selection() | forEach CHECK if there are available conditions to re-assign the discarded participant"
                                    );

                                // Filter those conditions where we have available slots. Create temporal list with true for available slots
                                condition_temp_array.push(
                                    condition_data.filter(function(
                                        value,
                                        index
                                    ) {
                                        return (
                                            key == value["task_name"] &&
                                            val == value["condition_name"] &&
                                            value["assigned_task"] <
                                                max_participants
                                        );
                                    }).length > 0
                        ...
                        if (debug_mode === true)
                            console.warn(
                                'condition_selection() | ELSE: if (available_conditions_ARRAY.length > 0) | actual_user.status === "assigned" | actual_user.status: ' +
                                    actual_user.status
                            );
                        ...
                        if (
                            typeof condition_temp_array !== "undefined" &&
                            condition_temp_array.includes(false)
                        ) {
                            experiment_blocked = true;
                            if (debug_mode === true)
                                console.warn(
                                    "condition_selection() | Final check DISCARDED | No available slots"
                                );
                            resolve(false);
                        } else {
                            experiment_blocked = false;
                            if (debug_mode === true)
                                console.warn(
                                    "condition_selection() | Final check DISCARDED |  Available slots. Participant can continue"
                                );
                            resolve(true);
                        }
                        // ----------------------------------------------
            ...
            function() {
                if (debug_mode === true)
                    console.warn(
                        "condition_selection() | Final check |  Error when loading conditions."
                    );
                reject(false);
            }
        );
...
/*
// convert a between array into a between dictionary --------------------------
function usable_between(task_name) {
    if (debug_mode === true) console.warn("usable_between()");

    let between_dict = {};
    let between_all_conditions = all_conditions[task_name];

    for (key of Object.keys(between_all_conditions)) {
        for (selection of between_selection) {
            if (between_all_conditions[key].includes(selection))
                between_dict[key] = selection;
        }
    }
...
function check_id_status(event) {
    ...
    if (
        params.has("uid") ||
        document.getElementById("input_uid").reportValidity()
    ) {
        ...
        XMLcall("findAll", "experimental_condition").then(function(
            condition_data
        ) {
            ...
            XMLcall("findRow", "user", {
                ...
                function(actual_user) {
                    // [[NEW PARTICIPANT]] (uid_external not in DB) -------------------------------
                    // ---------------------------------------------------------------------

                    if (
                        Object.keys(actual_user).length === 0 &&
                        actual_user.constructor === Object
                    ) {
                        if (debug_mode === true)
                            console.warn(
                                "check_id_status() | NEW participant " +
                        ...
                        condition_selection().then(function(accepted) {
                            ...
                            if (accepted) {
                                ...
                                console.warn(
                                    "NEW participant | available slots"
                                );
                            } else {
                                if (debug_mode === true)
                                    console.warn(
                                        "check_id_status() | NEW participant | condition_selection returned false"
                                    );
                                console.warn(
                                    "NEW participant | no available slots"
                                );
                            }
                        });
                        ...
                        if (debug_mode === true)
                            console.warn("check_id_status() | OLD participant");
                        ...
                        if (
                            actual_user.status == "assigned" ||
                            (actual_user.status == "discarded" &&
                                accept_discarded)
                        ) {
                            // GET **between_selection** conditions for the particicipant from the DB (so she can continue where she left off)
                            XMLcall("condition_selection", "", {
                                id: { id_user: uid }
                            }).then(function(between_selection_temp) {
                                ...
                                if (debug_mode === true)
                                    console.warn(
                                        "check_id_status() || between_selection (showing only first): '" +
                                            Object.values(
                                                between_selection_temp[0]
                                            )[0] +
                                            "'"
                                    );
              ...
              */
                                ...
                                for (
                                    var i = 0;
                                    i < between_selection_temp.length;
                                    i++
                                ) {
                                    let actual_task = Object.keys(
                                        between_selection_temp[i]
                                    )[0];
                                    let actual_condition = Object.values(
                                        between_selection_temp[i]
                                    )[0];

                                    // If the task exists in between_selection, add to array, else create array
                                    if (actual_task in between_selection) {
                                        between_selection[actual_task].push(
                                            actual_condition
                                        );
                                    } else {
                                ...
                                actual_time = new Date()
                                    .toISOString()
                                    .slice(0, 19);
                                DBtime = actual_user.start_date.replace(
                                    " ",
                                    "T"
                                ); // Date already stored in ISO format
                                seconds_since_start =
                                    (new Date(actual_time) - new Date(DBtime)) /
                                    1000;
                                ...
                                if (
                                    ...
                                    if (debug_mode === true)
                                        console.warn(
                                            "check_id_status() | ASSIGNED participant & seconds_since_start < max_sec"
                                        );
                                    user_assigned = true;
                                    ...
                                    if (debug_mode === true)
                                        console.warn(
                                            "check_id_status() || DISCARDED participant & accept_discarded OR ASSIGNED out of time & accept_discarded || actual_user.status: " +
                                                actual_user.status +
                                                " || out of time: " +
                                                (seconds_since_start > max_sec)
                                        );
                                    ...
                                    condition_selection(between_selection).then(
                                        function(accepted) {
                                            if (debug_mode === true)
                                                console.warn(
                                                    "check_id_status() || Inside condition_selection(). accepted: " +
                                                        accepted
                                                );
                                            ...
                                            if (accepted) {
                                                if (debug_mode === true)
                                                    console.warn(
                                                        "check_id_status() || OLD user || condition_selection() returned true"
                                                    );
                                                ...
                                                if (debug_mode === true)
                                                    console.warn(
                                                        'check_id_status() | UPDATE | status: assigned, start_date: actual_time, counter + 1, assigned_task + 1 | actual_user.status == "discarded" & accept_discarded === true || (seconds_since_start > max_sec && actual_user.status == "assigned" & accept_discarded === true) --> condition_selection(accepted)'
                                                    );
                                                console.warn(
                                                    "OLD participant | available slots, re-assign"
                                                );
                                                ...
                                                if (debug_mode === true)
                                                    console.warn(
                                                        "check_id_status() || OLD user || condition_selection() returned false"
                                                    );
                                                console.warn(
                                                    "OLD participant | no available slots"
                                                );
                                            }
                                        }
                                    );
                                    ...
                                    if (debug_mode === true)
                                        console.warn(
                                            "check_id_status() | IMPOSSIBLE CONDITION"
                                        );
                                ...
                                XMLcall(
                                    ...
                                    (elements = {
                                        sql: {
                                            sql_call:
                                                "SELECT task.task_name FROM user LEFT JOIN user_task USING (id_user) LEFT JOIN task USING (id_task)"
                                        },
                                        id: { id_user: uid }
                                    })
                                    ...
                                    if (debug_mode === true)
                                        console.warn(
                                            "check_id_status() [[ completed_experiments: " +
                                                completed_experiments.length +
                                                " || all_tasks: " +
                                                all_tasks.length +
                                                " ]]"
                                        );
                                    completed_experiments = answer.map(
                                        a => a.task_name
                                    ); // Extract all elements of object answer to completed_experiments
                                    script_loading(
                                        "tasks",
                                        all_tasks,
                                        completed_experiments
                            ...
                            console.warn(
                                "OLD participant | already completed the protocol"
                            );
                            ...
                            console.warn(
                                "OLD participant | discarded (!accept_discarded)"
                            );
...
function completed_task_storage(csv, task) {
    ...
    if (!user_assigned && !experiment_blocked) {
        XMLcall("findRow", "user", {
            ...
            function(actual_user) {
                ...
                if ("status" in actual_user) {
                    ...
                    if (actual_user.status == "discarded") {
                        XMLcall("findAll", "experimental_condition").then(
                            function(condition_data) {
                                ...
                                for (
                                    var i = 0;
                                    i < condition_data.length;
                                    i++
                                ) {
                                    if (
                                        !(
                                            condition_data[i].task_name in
                                            all_conditions_tasks
                                        )
                                    ) {
                                        all_conditions_tasks[
                                            condition_data[i].task_name
                                        ] = false;
                                    }
                                    if (
                                        condition_data[i].assigned_task >=
                                        max_participants
                                    ) {
                                ...
                                Object.keys(all_conditions_tasks).forEach(
                                    function(key) {
                                ...
                                if (!protocol_blocked && accept_discarded) {
                                    if (debug_mode === true)
                                        console.warn(
                                            'completed_task_storage() | actual_user.status == "discarded" --> !protocol_blocked && accept_discarded'
                                        );
                                    user_assigned = true;
                                    ...
                                    for (var [key, value] of Object.entries(
                                        ...
                                        for (var i = 0; i < value.length; i++) {
                                            ...
                                            ).then(function(actual_condition) {
                                                XMLcall(
                                                    "updateTable",
                                                    "experimental_condition",
                                                    {
                                    ...
                                    if (debug_mode === true)
                                        console.warn(
                                            'completed_task_storage() | UPDATE | status: assigned, start_date: actual_time, counter + 1, assigned_task + 1 | !user_assigned && !experiment_blocked --> actual_user.status == "discarded" --> !protocol_blocked && accept_discarded'
                                        );
                                } else {
                                    if (debug_mode === true)
                                        console.warn(
                                            "Participante bloqueado por límite en condiciones" +
                                                " #2"
                                        );
                    ...
                    if (debug_mode === true)
                        console.warn(
                            "completed_task_storage() || NEW user: first task"
                        );
                    ...
                    XMLcall("findAll", "experimental_condition").then(function(
                        ...
                        if (debug_mode == true)
                            console.warn(
                                "completed_task_storage() || Object.keys(between_selection).length " +
                                    Object.keys(between_selection).length
                            );
                        ...
                        if (Object.keys(between_selection).length == 0)
                        ...
                        for ( var i = 0; i < Object.keys(between_selection).length; i++ ) {
                            Object.keys(between_selection[Object.keys(between_selection)[i]]).forEach( selection => {
                                if (debug_mode == true)
                                    console.warn(
                                        "completed_task_storage() || LOOP between_selection"
                                    );
                                if (debug_mode == true)
                                    console.warn(between_selection);

                                // CHECK if available slots
                                completed_protocol_filtered = condition_data.filter(
                                    function(value, index) {
                                        return (
                                            /*
                                            value["assigned_task"] <
                                            ...
                                            value["condition_name"] ===
                                                between_selection[
                                                    Object.keys(between_selection)[
                                                        i
                                                    ]
                                                ][0]
                                                */
                                            value["condition_name"] == between_selection[Object.keys(between_selection)[i]][selection] && value["condition_key"] == selection && value["assigned_task"] < max_participants  && value["task_name"] == Object.keys(between_selection)[i]
                                        );
                                    }
                                );
                                ...
                                if (completed_protocol_filtered.length > 0) {
                                    ...
                                    if (debug_mode === true)
                                        console.warn(
                                            'completed_task_storage() | UPDATE | for(assigned_task + 1) | !user_assigned && !experiment_blocked --> ELSE "status" in actual_user'
                                        );
                                    protocol_blocked = false;
                                    ...
                                    if (debug_mode === true)
                                        console.warn(
                                            "completed_task_storage() | completed_protocol_filtered.length > 0 | NO available slots loop"
                                        );
                                    // alert("NO hay cupos disponibles");
                                    protocol_blocked = true;
                                    if (debug_mode === true)
                                        console.warn(
                                            "condition_selection() || Participante bloqueado por límite en condiciones" +
                                                " #3"
                                        ); // Ends up in jsPsych.end
                                    jsPsych.endExperiment(out_of_slots_message);
                                }
                        ...
                        if (!protocol_blocked) {
                            // Comes from for loop above

                            user_assigned = true;
                            added_task =
                                completed_protocol_filtered[0]["task_name"];
                            if (debug_mode === true)
                                console.warn(
                                    "completed_task_storage() || NEW user, first task | User assigned | task: " +
                                        added_task
                                );

                            // ADD data to user table
                            XMLcall("insertIntoTable", "user", {
                                dict: {
                                    id_protocol: pid,
                                ...
                                XMLcall("findRow", "user", {
                                    ...
                                    XMLcall("findRow", "task", {keys: ["task_name"], values: [task]}
                                        ).then(function(actual_task) {
                                    ...
                                    Object.keys(between_selection).forEach( task_key => { 
                                        Object.keys(between_selection[task_key]).forEach( 
                                            (condition_key) => {
                                                XMLcall("findRow", "experimental_condition", { keys: ["task_name", "condition_key", "condition_name"], values: [ task_key, condition_key, between_selection[task_key][condition_key]] }
                                                ).then(function(actual_condition) { 
                                    ...
                                    /*Object.entries(between_selection).forEach(
                                        ([key, value]) => {
                            ...
                            if (debug_mode === true)
                                console.warn(
                                    'completed_task_storage() | INSERT | table user, table user_task, table user_condition, counter + 1 | !user_assigned && !experiment_blocked --> ELSE "status" in actual_user --> !protocol_blocked'
                                );
        ...
        if (debug_mode === true)
            console.warn(
                "completed_task_storage() || asigned: second task and forward"
            );

        XMLcall("findRow", "user", { keys: ["id_user"], values: [uid] }).then(
            function(actual_user) {
                ...
                if ("status" in actual_user) {
                    ...
                    if (actual_user.status == "assigned") {
                        ...
                        if (!accept_discarded) {
                            ...
                            hours_until_discarded =
                                Math.round(
                                    ((max_sec - seconds_since_start) / 3600 +
                                        Number.EPSILON) *
                                        100
                                ) / 100;
                            minutes_until_discarded =
                                Math.round(
                                    ((max_sec - seconds_since_start) / 60 +
                                        Number.EPSILON) *
                                        100
                                ) / 100;
                            ...
                            if (seconds_since_start > max_sec) {
                                ...
                                XMLcall("findAll", "user_condition", {
                                    ...
                                    for ( var i = 0;
                                        ...
                                        if (debug_mode === true)
                                            console.warn(
                                                "completed_task_storage() || DISCARD PARTICIPANT || updateTable: experimental_condition || assigned_task: assigned_task - 1" +
                                                    new Date()
                                                        .toISOString()
                                                        .slice(0, 19)
                                            );
                                ...
                                if (debug_mode === true)
                                    console.warn(
                                        'completed_task_storage() | UPDATE | status: discarded, counter - 1, for(assigned_task - 1) | actual_user.status == "assigned" --> !accept_discarded --> seconds_since_start > max_sec'
                                    );
                                if (debug_mode === true)
                                    console.warn(
                                        "completed_task_storage() | OUT OF TIME | actual_time: " +
                                            actual_time +
                                            " | DBtime" +
                                            DBtime +
                                            " | Started " +
                                            seconds_since_start +
                                            " seconds ago | Time ends in " +
                                            hours_until_discarded +
                                            " hours [" +
                                            minutes_until_discarded +
                                            " minutes]"
                            ...
                            if (debug_mode === true)
                                console.warn(
                                    "completed_task_storage() | assigned & accept_discarded | continue"
                                );
                        }
                        ...
                        if (!last_task) {
                            if (debug_mode === true)
                                console.warn(
                                    "completed_task_storage() | User discarded: actual_user.status != assigned --> !last_task | jsPsych.endExperiment()"
                                );
                            jsPsych.endExperiment(
                                discarded_user_no_time_message
                            );
                            ...
                            if (debug_mode === true)
                                console.warn(
                                    "completed_task_storage() | User discarded: actual_user.status != assigned --> last_task | continue"
                                );
                        }
                    }
                    ...
                    if (
                        ...
                        if (debug_mode === true)
                            console.warn(
                                'completed_task_storage() | insertIntoTable id_user-id_task | actual_user.status == "assigned" || actual_user.status == "completed" |'
                            );
                    }
                    // --------------------------------------------------------------------------------------------------------
                    ...
                    if (debug_mode === true)
                        console.warn(
                            "completed_task_storage() | Participant not found"
                        );
                }
            },
            function(user_not_found) {
                if (debug_mode === true)
                    console.warn(
                        "completed_task_storage() | Error in DB (searching user)"
                    );
            }
        );
    }
