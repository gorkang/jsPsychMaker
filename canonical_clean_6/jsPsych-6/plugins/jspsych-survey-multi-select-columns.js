...
jsPsych.plugins['survey-multi-select-columns'] = (function() {
  var plugin = {};

  plugin.info = {
    name: 'survey-multi-select-columns',
    description: '',
    parameters: {
      questions: {
        ...
        nested: {
          ...
          columns: {
            ...
            pretty_name: 'columns',
            default: 0,
            description: 'Sets columns number.'
          },
          name: {
            type: jsPsych.plugins.parameterType.STRING,
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if (question.horizontal || question.columns != 0) {
	...
	console.warn('question.columns: ' + question.columns);
        console.warn(75/(question.columns));
        console.warn('width disponible para contenido: ' + document.getElementById("jspsych-content").offsetWidth)
        console.warn('window.innerWidth: ' + window.innerWidth)
        console.warn('screen.width: ' + screen.width)
        console.warn('MAX columnas/145: ' + Math.floor(document.getElementById("jspsych-content").offsetWidth/145))
        
        //alert(document.getElementById("jspsych-content").offsetWidth)
        
        var available_resolution = document.getElementById("jspsych-content").offsetWidth
        if (screen.width < available_resolution) available_resolution = screen.width
        var MAX_cols = Math.floor(available_resolution/145)
        if (MAX_cols < question.columns) question.columns = MAX_cols
      	console.warn('window.innerWidth POST: ' + window.innerWidth)
        console.warn('screen.width POST: ' + screen.width)
      	console.warn('question.columns POST: ' + question.columns);
      ...
      for (var j = 0; j < question.options.length; j++) {
        ...
        */
        ...
        display_element.querySelector(question_selector).innerHTML += '<div id="'+option_id_name+'" class="'+_join(plugin_id_name, 'option')+'"' +  (question.columns != 0 ? ' style="width:' + (75/(question.columns)).toString() + '%"' : '') + '></div>';
