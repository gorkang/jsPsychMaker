...
var question02 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">Imagine we are throwing a five-sided die 50 times. On average, out of these 50 throws how many times would this five-sided die show an odd number (1, 3 or 5)?</div>', type: 'number', range: [0, 50], required: true, error_text: 'Please enter a valid number between 0 and 50'}],
...
var if_question02 = {
  ...
  conditional_function: function(){
    let data = (JSON.parse((jsPsych.data.get().values().find(x => x.trialid === 'BNTen_01'))['response'])['Q0']).trim();
    if((data) !=  '25'){
      return true;
    } else {
      return false;
...
var question03 = {
  type: 'survey-text',
  questions: [{prompt: '<div class="justified">Imagine we are throwing a loaded die (6 sides). The probability that the die shows a 6 is twice as high as the probability of each of the other numbers. On average, out of these 70 throws how many times would the die show the number 6?</div>', type: 'number', range: [0, 70], required: true, error_text: 'Please enter a valid number between 0 and 70'}],
...
var if_question04 = {
  ...
  conditional_function: function(){
    try {
      let data = (JSON.parse((jsPsych.data.get().values().find(x => x.trialid === 'BNTen_03'))['response'])['Q0']).trim();
