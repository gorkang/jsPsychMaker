...
var question07 = {
  ...
  questions: [{prompt: '<div class="justified">¿Cuál es su religión?</div>', required: true}],
...
var if_question07 = {
  ...
  conditional_function: function(){
    let data = ((jsPsych.data.get().values().find(x => x.trialid === 'OTRASRELIG_06'))['response']['Q0']).toString().trim();
...
var question09 = {
  type: jsPsychSurveyText,
  questions: [{prompt: '<div class="justified">¿Cuál es su preferencia y/o afiliación religiosa?</div>', required: true}],
...
var if_question09 = {
  ...
  conditional_function: function(){
    let data = ((jsPsych.data.get().values().find(x => x.trialid === 'OTRASRELIG_08'))['response']['Q0']).toString().trim();
    if((data) ==  'Otro'){
      return true;
    } else {
      return false;
