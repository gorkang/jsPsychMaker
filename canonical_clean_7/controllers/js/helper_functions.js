...
// jsPsych timeline for questions, used in jsPsych.run(questions) (start_protocol function).
var questions = []
...
function input_reload () {
  inputs = [];
  for (let actual_input of document.getElementsByTagName("input")) {
    if (actual_input.type == 'text') {
      inputs.push(actual_input);
    }
  }

  let actual_trial_questions = jsPsych.current_trial.questions
  for (let i = 0; i < actual_trial_questions.length; i++) {

    // name is required in questions
    if (actual_input = inputs.find(element => element.dataset.name === actual_trial_questions[i].name)) {
      if ('input_type' in actual_trial_questions[i]) {
        actual_input.type = actual_trial_questions[i].input_type;
      }
      if ('min' in actual_trial_questions[i]) {
        actual_input.min = actual_trial_questions[i].min
      }
      if ('max' in actual_trial_questions[i]) {
        actual_input.max = actual_trial_questions[i].max
      }
    }
  }
}
