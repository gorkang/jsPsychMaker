    ...
    display_element.querySelector('#jspsych-survey-text-next').addEventListener('click', function(event) {
      ...
      for(var index=0; index<matches.length; index++){
        ...
        if (trial.questions[index].type == "number" || trial.questions[index].type == "range") {
          ...
          if (trial.questions[index].language == "spanish" && (min != -Infinity || max != Infinity) ) {
            text_validation = " (";

            if (min != -Infinity)
              text_validation += "mínimo " + min;

            if (min != -Infinity && max != Infinity)
              text_validation += ", ";

            if (max != Infinity)
              text_validation += "máximo " + max ;
        ...
        if (trial.questions[index].type == 'text') {
          if (val.length >= min && val.length <= max) {
            ...
            if (trial.questions[index].language == "spanish" && (min != -Infinity || max != -Infinity)) {
              ...
              if (min != -Infinity)
                text_validation += "mínimo " + min + " carácteres";

              if (min != -Infinity && max != Infinity)
                text_validation += ", ";

              if (max != Infinity) {
                text_validation += "máximo " + max + " carácteres";
              }
        ...
        if ( (validation && pass) || !required ) {
          ...
          textBox.focus();
          var message = '';
          if (trial.questions[index].error_text === "" || trial.questions[index].error_text === undefined) {
            ...
            display_element.querySelector(".fail-message").innerHTML = '<span style="color: red;" class="required">' + message + text_validation +'.</span>';
          } else {
