...
// Randomly assign a Between variable
  var between_selection = []
  between_selection.push(jsPsych.randomization.factorial(between_var, 1)[0].type);

// Randomize order of Within variables #TODO
  var full_design = jsPsych.randomization.factorial(within_var, 1, false, false);
...
// first option between block --------------------------------------------------
  ...
  var instructions_between_01 = {
      type: 'instructions',
      pages: ['Instrucciones ' + data_type[between_selection[0]]["text"]], //se agrega el texto de la variable between seleccionada
      data: {trialid: 'Instructions',
            condition_between: between_selection, procedure: 'Between-Within'},
    ...
    timeline: [
      ...
      // SCREEN 2: Disease and test description --------------------------------
      {
        ...
        html: function() {
          ...
          } else if (between_selection[0] == 'Text') { // que pasa si las imagenes tienen distintos tamaños? #TODO
...
questions.push({
    ...
    func: function(){
      ...
      saveData(data, online, 'Between-Within');
    },
    procedure: 'Between-Within'
});
